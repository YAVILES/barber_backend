...
from barber.utils.functions import format_headers_import
...
class UserFilter(filters.FilterSet):
    class Meta:
        model = User
        fields = ['status', 'name', 'email', 'username']
...
class UserViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        only_barbers = self.request.query_params.get('only_barbers', None)
        queryset = self.queryset

        if only_barbers:
            queryset = queryset.filter(is_barber=True)
        ...
        # Barber can see everything except superusers
